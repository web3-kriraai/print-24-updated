...
import { getSlotsForDesigner } from '../controllers/availability.controller.js';
...
export async function bookVisit(customerId, body) {
    ...
    // Allow 'paid', 'completed', or 'pending'
    if (status !== 'paid' && status !== 'pending' && status !== 'completed') {
        throw Object.assign(
            new Error(`Invalid payment status for booking: ${rawStatus}. Expected Paid, Completed, or Pending.`),
            { statusCode: 400 }
        );
    }
    ...
    if (visitLocation === 'OFFICE') {
        if (!isFullDay) {
            ...
            // Check if a full-day booking exists for this date
            const fullDayBooking = await PhysicalDesignerBooking.findOne({
                designerId,
                visitDate: normalizedDate,
                isFullDay: true,
                visitStatus: { $in: ['Scheduled', 'Accepted', 'InProgress'] }
            ...
            if (fullDayBooking) {
                throw Object.assign(new Error('Designer is booked for the full day on this date.'), { statusCode: 409 });
            }
        } else {
            // If booking a FULL DAY, no other bookings should exist
            const existingAny = await PhysicalDesignerBooking.findOne({
                designerId,
                visitDate: normalizedDate,
                visitStatus: { $in: ['Scheduled', 'Accepted', 'InProgress'] }
            });
            if (existingAny) {
                throw Object.assign(new Error('Cannot book full day: designer already has bookings for this date.'), { statusCode: 409 });
            }
        }
    }
...
export async function getDesignerSlots(designerId, dateStr) {
    return await getSlotsForDesigner(designerId, dateStr);
}
