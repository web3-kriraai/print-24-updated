    ...
    try {
        ...
        if (pricingResult.success === false) {
            return res.status(200).json({
                success: false,
                isAvailable: false,
                errorCode: "PRICE_NOT_SET",
                message: pricingResult.message || "Price not set for this product",
                displayMessage: "Price is not given yet",
                meta: {
        ...
        const prices = results.map((result, index) => {
            if (result.error || result.success === false) {
                return {
                    ...
                    success: false,
                    error: result.message || "Price not available",
                };
            }
