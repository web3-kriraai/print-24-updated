...
export const updateUserRole = async (req, res) => {
  try {
    ...
    // Map frontend 'employee' role to backend 'emp' role
    if (role === 'employee') {
      role = 'emp';
    }

    // Allow 'admin', 'user', 'emp' (and 'employee' processed above)
    ...
    return res.json({
      ...
      user: {
        ...
        name: user.name,
        email: user.email,
        role: user.role === 'emp' ? 'employee' : user.role, // Return normalized role to frontend if needed, but keeping DB value is standard
      },
    });
  } catch (error) {
