...
export const createSubCategory = async (req, res) => {
  try {
    ...
    if (sortOrderProvided) {
      ...
      // Auto-increment based on scope (parent or category)
      let query;
      if (parentSubcategory) {
        // Nested subcategory - find max among siblings (same parent)
        query = { parent: parentSubcategory._id };
      } else {
        // Top-level subcategory - find max among subcategories in same category
        query = {
          category: categoryId,
          $or: [
            { parent: null },
      ...
      const maxSortOrderDoc = await SubCategory.findOne(query)
        .sort({ sortOrder: -1 })
        .select('sortOrder')
        .lean();
