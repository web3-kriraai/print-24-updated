...
export async function triggerBulkProcessingIfNeeded(updatedOrderOrId) {
    ...
    try {
        console.log('💳 [Webhook] Cascading payment status to child orders for:', updatedOrder.orderNumber);

        if (updatedOrder.childOrders && updatedOrder.childOrders.length > 0) {
            ...
            console.log(`✅ [Webhook] Cascaded payment to ${result.modifiedCount} child orders.`);
        } else {
            console.warn(`⚠️ [Webhook] Parent order ${updatedOrder.orderNumber} has no child orders array.`);
        }
    } catch (err) {
        console.error('[Webhook] Failed to cascade payment to child orders:', err);
    }
}
...
export const handleWebhook = async (req, res) => {
    ...
    try {
        ...
        console.log(`🔍 Detected Gateway: ${gatewayName}`);
        console.log('═══════════════════════════════════════════════════════');
        ...
        console.log('ℹ️ Returning JSON response (No redirect match)');
...
function determineGateway(req) {
    ...
    if (req.path?.includes('/payu') || req.originalUrl?.includes('/payu')) {
        return 'PAYU';
    }
...
async function processPayUCallback(payload, provider) {
    ...
    if (status === 'success') {
        ...
        if (transaction.status !== 'SUCCESS') {
            ...
            if (transaction.order) {
                ...
                try {
                    ...
                    // Don't throw here, just log error so redirect still happens
                    // throw updateError; 
                }
            } else {
                console.log('ℹ️ Test payment (no order ID), skipping order update.');
            }
        }



        // Clean up frontend URL (remove trailing slash)
        const frontendUrl = (process.env.FRONTEND_URL || 'http://localhost:5173').replace(/\/$/, '');

        let redirectPath;
        if (transaction.order) {
            redirectPath = `/order/${transaction.order}`;
        } else {
            // Fallback for test payments
            redirectPath = `/admin/test-payment`;
        }

        // Add query params
        const finalUrl = `${frontendUrl}${redirectPath}?payment=success&transaction_id=${transaction._id}`;
        ...
        return {
            ...
            transaction: transaction._id,
            order: transaction.order,
            redirect: `${frontendUrl}${redirectPath}?payment=failed&transaction_id=${transaction._id}`
        };
        ...
        return {
            ...
            transaction: transaction._id,
            status: 'pending',
            redirect: `${frontendUrl}${redirectPath}?payment=pending&transaction_id=${transaction._id}`
        };
    }
}
