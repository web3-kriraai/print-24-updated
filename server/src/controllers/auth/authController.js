  ...
  try {
    ...
    if (existingUser && existingUser.password && existingUser.password.length > 0 &&
      ...
      return res.status(409).json({
        ...
        conflictField: "emailAddress",
      });
    }

    // Check if mobile number already exists (and is a complete account, excluding current user)
    // Normalize mobile number for search (handle +91, spaces)
    const normalizedMobileForCheck = mobileNumber.replace(/\D/g, "").slice(-10);
    const mobileQuery = {
      $or: [
        { mobileNumber: { $regex: new RegExp(`${normalizedMobileForCheck}$`) } },
    ...
    if (gstNumber && gstNumber.trim()) {
      const gstQuery = { gstNumber: gstNumber.trim().toUpperCase() };
      if (existingUser) {
  ...
  try {
    ...
    const mobileQuery = {
      $or: [
        ...
        { mobileNumber: normalizedMobile.startsWith('+91') ? normalizedMobile.slice(3) : normalizedMobile },
        { mobileNumber: !normalizedMobile.startsWith('+') ? `+91${normalizedMobile}` : normalizedMobile }
      ],
      password: { $exists: true, $ne: '' }
    };
    if (existingUser) {
