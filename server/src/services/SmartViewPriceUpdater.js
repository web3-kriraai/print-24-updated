...
class SmartViewPriceUpdater {
  ...
  async updateMasterPrices(priceUpdates, resolutionStrategy) {
    ...
    for (const update of priceUpdates) {
      ...
      if (conflicts.hasConflicts) {
        ...
        // Add impact summary for UI (assuming single product update flow mostly)
        // If multiple products have conflicts, this might overwrite, but usually we handle one modal at a time
        results.impactSummary = {
          product: {
            _id: productId,
            name: conflicts.productName || 'Product',
            sku: conflicts.productSku || ''
          },
          updateLevel: 'MASTER',
          currentMasterPrice: conflicts.currentMasterPrice || 0,
          newPrice: newPrice,
          totalAffectedItems: conflicts.conflicts.length,
          affectedSegments: []
        };

        // Prepare affectedItems for frontend (New Format) to ensure correct display
        results.affectedItems = conflicts.conflicts.map(c => {
          const diff = c.newPrice - c.existingPrice;
          let percent = '0%';
          if (c.existingPrice !== 0) {
            percent = ((diff / c.existingPrice) * 100).toFixed(1) + '%';
          }

          return {
            priceBook: { name: c.priceBookName },
            zone: c.zone ? { name: c.zoneName } : null,
            segment: c.segment ? { name: c.segmentName } : null,
            pricing: {
        ...
        // Delegate to VirtualPriceBookService for consistent resolution
        await this.virtualPriceBookService.applyResolution(
          resolutionStrategy,
          conflicts.conflicts,
          newPrice,
  ...
  async detectChildConflicts(zoneId, segmentId, productId, newPrice) {
    ...
    for (const entry of allEntries) {
      ...
      // Build robust IDs for comparison
      const bookZoneId = book.zone?._id?.toString() || (book.zone?.toString().startsWith('{') ? null : book.zone?.toString()) || null;
      const bookSegmentId = book.segment?._id?.toString() || (book.segment?.toString().startsWith('{') ? null : book.segment?.toString()) || null;
      const targetZoneId = zoneId?.toString() || null;
      const targetSegmentId = segmentId?.toString() || null;

      // Skip if it's the exact same context (prevent conflict with self)
      if (bookZoneId === targetZoneId && bookSegmentId === targetSegmentId) continue;

      // Check if this is a more specific (child) context
      const isChild = this.isChildContext(targetZoneId, targetSegmentId, bookZoneId, bookSegmentId);

      if (isChild) {
        console.log(`⚠️ Conflict Found: Book "${book.name}" (Price: ${entry.basePrice}) vs New ${newPrice}`);

        conflicts.push({
          type: bookZoneId && bookSegmentId ? 'ZONE_SEGMENT_OVERRIDE' :
            bookZoneId ? 'ZONE_OVERRIDE' : 'SEGMENT_OVERRIDE',
          existingPrice: entry.basePrice || 0,
          newPrice,
          priceBookId: book._id,
          priceBookName: book.name || 'Unknown Price Book',
          zone: book.zone,
          zoneName: book.zone?.name,
          segment: book.segment,
          segmentName: book.segment?.name
        });
      }
    }
  ...
  async detectAllChildConflicts(productId, newPrice) {
    ...
    console.log(`[ConflictDetection] Checking ${allEntries.length} entries for ${productId}`);

    const seenBooks = new Set();
    let currentMasterPrice = 0;
    let productName = 'Product';
    let productSku = '';

    // First pass: find master price and product info
    if (allEntries.length > 0 && allEntries[0].product) {
      productName = allEntries[0].product.name;
      productSku = allEntries[0].product.sku;
    }

    let masterEntry = allEntries.find(e => e.priceBook?._id?.toString() === masterBook?._id?.toString());

    // Fallback: If master entry not in bulk fetch, try explicit fetch
    if (!masterEntry) {
      // console.log('[ConflictDetection] Master entry not found in bulk fetch, checking DB directly...');
      masterEntry = await PriceBookEntry.findOne({ priceBook: masterBook._id, product: productId }).lean();
    }

    if (masterEntry) {
      currentMasterPrice = masterEntry.basePrice;
    } else {
      console.warn(`[ConflictDetection] ⚠️ No existing master price found for ${productId}`);
    }
    ...
    for (const entry of allEntries) {
      ...
      // Deduplicate conflicts by pricebook ID
      ...
      conflicts.push({
        ...
        priceBookName: book.name || 'Unknown Price Book',
        zone: book.zone,
        zoneName: book.zone?.name || 'All Zones',
        segment: book.segment,
        segmentName: book.segment?.name || 'All Segments' // Fallback for "Unknown"
      });
    }
