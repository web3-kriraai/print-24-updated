...
class VirtualPriceBookService {
  ...
  async calculateVirtualPrice(productId, zoneId, segmentId, contextOverride = {}) {
    try {
      ...
      // 0. Check product availability in zone hierarchy
      const geoZoneHierarchy = contextOverride.geoZoneHierarchy || [];
      if (geoZoneHierarchy.length > 0 || zoneId) {
        const availability = await this.checkProductAvailability(productId, zoneId, geoZoneHierarchy);
        if (!availability.isAvailable) {
          console.log(`❌ Product NOT AVAILABLE in hierarchy: ${availability.reason}`);
          return {
            productId,
            zoneId,
            segmentId,
      ...
      if (!masterPrice) {
        console.log(`❌ No master price found for product ${productId}`);
        return {
          productId,
          zoneId,
          segmentId,
          isAvailable: false,
          availabilityReason: "Price is not given yet",
          masterPrice: null,
          finalPrice: null,
          adjustments: [],
          modifiersApplied: 0,
      ...
      if (!context.product && !context.categoryId && !context.category) {
        ...
        try {
          const productData = await Product.findById(productId).select('category subcategory productType slug dynamicAttributes').lean();
          if (productData) {
            context.categoryId = productData.category;
            context.subCategoryId = productData.subcategory;
            context.productType = productData.productType;
            context.sku = productData.slug;
            // Map dynamicAttributes from version-1 format if needed, or leave empty if mainly for options
            // context.productAttributes = productData.dynamicAttributes || []; 
          }
        } catch (e) {
          console.warn('Failed to fetch product details for context', e);
        }
      } else if (context.product) {
        // Populate context fields from provided product object
        context.categoryId = context.product.category;
        context.subCategoryId = context.product.subcategory;
        context.productType = context.product.productType;
        context.sku = context.product.slug;
      }
  ...
  async getApplicableModifiers(productId, zoneId, segmentId, context) {
    ...
    // Get zone-specific modifiers (Hierarchical)
    const geoZoneHierarchy = context.geoZoneHierarchy || [];
    const zonesToSearch = geoZoneHierarchy.length > 0
      ? geoZoneHierarchy.map(z => z._id)
      : (zoneId ? [zoneId] : []);

    if (zonesToSearch.length > 0) {
      const zoneModifiers = await PriceModifier.find({
        appliesTo: 'ZONE',
        geoZone: { $in: zonesToSearch },
        isActive: true
  ...
  /**
   * Check if product is available in the given zone hierarchy
   * (Waterfall: City -> District -> Zone -> Country)
   */
  async checkProductAvailability(productId, zoneId, geoZoneHierarchy = []) {
    try {
      // If hierarchy is provided, check each level (most specific first)
      const zonesToCheck = geoZoneHierarchy.length > 0
        ? geoZoneHierarchy.map(z => z._id)
        : (zoneId ? [zoneId] : []);

      if (zonesToCheck.length === 0) return { isAvailable: true };

      console.log(`🔍 Checking availability waterfall for ${zonesToCheck.length} zones...`);

      for (const id of zonesToCheck) {
        ...
        if (restriction && restriction.isSellable === false) {
          return {
            isAvailable: false,
            reason: restriction.reason || `Product not available in zone ${id}`
          };
        }
      }
      ...
      console.error('Error checking product availability hierarchy:', error);
  ...
  async getSmartView(filters) {
    ...
    if (zone && segment && product) {
      ...
      return await this.getMasterView(filters.page, filters.limit);
    }
  }
  ...
  async getZoneView(zoneId) {
    ...
    let masterEntries = [];
    if (masterBook) {
      masterEntries = await PriceBookEntry.find({
        priceBook: masterBook._id,
   ...
   */
  async getMasterView(page = 1, limit = 50) {
    const masterBook = await PriceBook.getMasterBook();
    if (!masterBook) {
      // Return empty if no master book
      return {
        viewType: 'MASTER',
        masterBookId: null,
        masterBookName: 'No Master Price Book Found',
        entries: [],
        pagination: {
          page,
          limit,
    ...
    const [entries, totalCount] = await Promise.all([
      PriceBookEntry.find({ priceBook: masterBook._id })
        .populate('product')
        .skip(skip)
        .limit(limit)
        .lean(),
      PriceBookEntry.countDocuments({ priceBook: masterBook._id })
    ]);
  ...
  async applyOverwrite(conflicts, newPrice, zoneId, segmentId, productId) {
    ...
    let book;
    if (!zoneId && !segmentId) {
      book = await PriceBook.getMasterBook();
    } else {
      book = await this.getOrCreateBookForContext(zoneId, segmentId);
    }

    if (!book) throw new Error('Could not resolve target price book');
