...
import { API_BASE_URL_WITH_API, getAuthHeaders } from '../lib/apiConfig';
...
const PaymentTestPage = () => {
    ...
    const fetchGateways = async () => {
        try {
            const response = await fetch(`${API_BASE_URL_WITH_API}/admin/payment-gateways`, {
            ...
            setGateways(gatewayList.filter((g: any) => g.is_active));
        } catch (error) {
    ...
    // Check for PayU redirect params
    useEffect(() => {
    ...
    const initializeTestPayment = async () => {
        ...
        try {
            ...
            const paymentResponse = await fetch(`${API_BASE_URL_WITH_API}/payment/test-initialize`, {
                method: 'POST',
    ...
    const showRazorpayCheckout = async (paymentData: any) => {
        ...
        const options = {
            ...
            handler: async function (response: any) {
                ...
                // Verify payment
                try {
                    const verifyResponse = await fetch(`${API_BASE_URL_WITH_API}/payment/verify`, {
                        method: 'POST',
                        headers: getAuthHeaders(true) as any,
                        body: JSON.stringify({
                    ...
                    if (result.success) {
                        alert('✅ Payment Successful!\nPayment ID: ' + (response.razorpay_payment_id || result.transaction_id));
                    } else {
