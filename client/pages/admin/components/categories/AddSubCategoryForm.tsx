...
import { AlertCircle, FileText, Image as ImageIcon, Loader, Plus, Save, X, Sparkles, ChevronRight } from 'lucide-react';
...
const AddSubCategoryForm: React.FC<AddSubCategoryFormProps> = ({
    ...
    // Auto-calculate sort order when category changes
    useEffect(() => {
        if (subCategoryForm.category) {
            const existingSubCats = subCategories.filter(sc =>
                (typeof sc.category === 'object' ? sc.category?._id : sc.category) === subCategoryForm.category &&
                !sc.parent
            );

            const maxSortOrder = existingSubCats.reduce((max, sc) => Math.max(max, sc.sortOrder || 0), 0);
            setSubCategoryForm({
                ...subCategoryForm,
                sortOrder: maxSortOrder + 1
            });
        }
    }, [subCategoryForm.category, subCategories]);
    ...
    return (
        ...
        >
            ...
            <motion.div
                ...
                className="relative overflow-hidden bg-gradient-to-br from-rose-50 via-white to-rose-50 rounded-xl p-4 md:p-6 text-slate-800 shadow-sm border border-rose-100"
            >
                <div className="relative z-10 flex items-center gap-4">
                    <motion.div
                        ...
                        className="w-12 h-12 bg-gradient-to-br from-rose-100 to-white backdrop-blur-sm rounded-xl flex items-center justify-center border border-rose-200 shadow-sm"
                    >
            ...
            <div className="bg-white rounded-xl shadow-sm p-4 md:p-6 border border-slate-200">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className={`w-full px-3 py-2.5 text-sm bg-white border rounded-lg focus:ring-2 transition-all ${subCategoryFormErrors.name
                                ...
                                : 'border-slate-300 hover:border-slate-400 focus:border-rose-400 focus:ring-rose-100'
                                }`}
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <input
                                ...
                                className="w-full pl-3 pr-9 py-2.5 text-sm bg-slate-50 border border-slate-300 rounded-lg focus:border-rose-400 focus:ring-2 focus:ring-rose-100 transition-all font-mono"
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                            Auto-calculated based on parent category
                        </p>
                    </div>
                    ...
                    <div className="md:col-span-2">
                        ...
                        <motion.div
                            ...
                            className="relative"
                        >
                            <label className={`flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer transition-all overflow-hidden group ${subCategoryForm.image ? 'border-rose-400 bg-rose-50' : 'border-slate-300 hover:border-rose-400 hover:bg-slate-50'
                                }`}
                            >
                                <AnimatePresence mode="wait">
                                    {subCategoryForm.image ? (
                                        ...
                                        >
                                            ...
                                            >
                                                <ImageIcon className="w-8 h-8 mb-2 text-slate-400 group-hover:text-rose-500 transition-colors" />
                                            </motion.div>
                ...
                >
                    <motion.button
                        ...
                        className="px-6 py-3 bg-gradient-to-r from-rose-500 to-pink-500 text-white rounded-lg hover:shadow-sm transition-all duration-200 disabled:opacity-50 flex items-center justify-center gap-2 shadow-sm text-sm font-medium"
                    >
                        {loading ? (
                            <>
                                <motion.div
