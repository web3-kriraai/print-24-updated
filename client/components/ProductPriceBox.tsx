...
interface ProductPriceBoxProps {
  ...
  priceData?: any; // Start off with optional external price data
}
...
export default function ProductPriceBox({
  ...
  useEffect(() => {
    // If external price data is provided, use it instead of fetching
    if (priceData) {
      if (priceData.pricing) setPricing(priceData.pricing);
      if (priceData.meta) setPricingMeta(priceData.meta);
      setLoading(false);
      return;
    }
  ...
  }, [productId, JSON.stringify(selectedDynamicAttributes), quantity, userContext, contextLoading, priceData]);
            ...
            <div className="mt-3 p-3 bg-linear-to-r from-blue-50 to-green-50 rounded-lg border border-gray-200">
              <div className="flex flex-wrap items-center gap-2 text-sm">
                ...
                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded font-semibold">
                  👤 {pricingMeta?.userSegmentName || userContext.segment.name}
                </span>
                {(pricingMeta?.geoZone || pricingMeta?.usedZoneName || userContext.location.geoZone.name) && (
                  <span className="px-2 py-1 bg-green-100 text-green-800 rounded font-semibold">
                    📍 {pricingMeta?.geoZone || pricingMeta?.usedZoneName || userContext.location.geoZone.name}
                  </span>
                )}
                {(pricingMeta?.pincode || userContext.location.pincode) && (
                  <span className="text-xs text-gray-500">({pricingMeta?.pincode || userContext.location.pincode})</span>
                )}
              </div>
