...
const PaymentGatewaySelector: React.FC<PaymentGatewaySelectorProps> = ({
    ...
    const fetchGateways = async () => {
        try {
            ...
            if (activeGateways.length > 0) {
                const sorted = [...activeGateways].sort((a, b) => a.priority - b.priority);
                const topPriority = sorted[0];
                setSelectedGateway(topPriority.name);

                // Auto-trigger payment if enabled
                if (autoTrigger && !autoTriggeredRef.current) {
                    autoTriggeredRef.current = true;
                    // Small delay to let state settle
                    setTimeout(() => {
                        handlePaymentWithGateway(topPriority.name);
                    }, 500);
                }
            }
        } catch (error) {
    ...
    const handlePaymentWithGateway = async (gatewayName: string) => {
        if (!gatewayName) {
    ...
    const handlePayment = () => handlePaymentWithGateway(selectedGateway);
