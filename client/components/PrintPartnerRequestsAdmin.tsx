// Print Partner Requests Admin Component
// This component displays print partner signup requests for admin approval/rejection

import React, { useState, useEffect } from "react";
import {
    Briefcase, CheckCircle, XCircle, User, Mail, Phone, MapPin,
    FileText, Eye, Clock, RefreshCw, Search, ChevronLeft, ChevronRight, X
} from "lucide-react";
import { API_BASE_URL_WITH_API as API_BASE_URL } from "../lib/apiConfig";

// Print Partner Request interface matching PrintPartnerProfile model
...
interface PrintPartnerRequestsAdminProps {
    onSuccess?: (message: string) => void;
    onError?: (message: string) => void;
}

const PrintPartnerRequestsAdmin: React.FC<PrintPartnerRequestsAdminProps> = ({ onSuccess, onError }) => {
    const [requests, setRequests] = useState<PrintPartnerRequest[]>([]);
    ...
    // Modal State
    const [selectedRequest, setSelectedRequest] = useState<PrintPartnerRequest | null>(null);
    ...
    const fetchRequests = async () => {
        ...
        try {
            ...
            console.error("Fetch print partner requests error:", error);
            onError?.(error.message || "Failed to fetch print partner requests");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleApprove = async (requestId: string) => {
        ...
        try {
            ...
            const data = await response.json();
            if (data.success) {
                onSuccess?.("Print partner request approved successfully!");
                fetchRequests();
    ...
    const handleReject = async (requestId: string) => {
        ...
        try {
            ...
            const data = await response.json();
            if (data.success) {
                onSuccess?.("Print partner request rejected successfully!");
                fetchRequests();
                if (selectedRequest?._id === requestId) setSelectedRequest(null);
            } else {
            ...
            <div className="glass-panel p-4 rounded-2xl bg-white shadow-sm border border-gray-100">
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-gray-900">Print Partners</h2>
                            <p className="text-xs text-gray-500">Manage partnership requests</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                        ...
                        <select
                            ...
                            className="w-full sm:w-auto px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm bg-white"
                        >
                            ...
                            {loading ? (
                                ...
                                currentItems.map((request) => (
                                    <tr key={request._id} className="hover:bg-gray-50/50 transition-colors cursor-pointer" onClick={() => setSelectedRequest(request)}>
                                        ...
                                        <td className="px-6 py-4 font-medium text-gray-900">{request.businessName}</td>
                                        <td className="px-6 py-4 text-gray-600">{request.ownerName}</td>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
                        <div className="sticky top-0 bg-white border-b border-gray-100 p-6 flex items-center justify-between z-10">
                            <div>
                                ...
                                <p className="text-sm text-gray-500">Full information for {selectedRequest.businessName}</p>
                            </div>
                            <button
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    ...
                                    <div className="p-4 rounded-xl bg-gray-50 space-y-3 text-sm">
                                        <div className="flex flex-col">
                                            <span className="text-gray-500 text-xs uppercase font-bold tracking-wider">Business Name</span>
                                            <span className="font-medium text-gray-900 text-lg">{selectedRequest.businessName}</span>
                                        </div>
                                        <div className="flex flex-col">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="p-4 rounded-xl bg-gray-50 space-y-3 text-sm">
                                        <div className="flex flex-col">
                                            <span className="text-gray-500 text-xs uppercase font-bold tracking-wider">Owner Name</span>
                                            <span className="font-medium text-gray-900">{selectedRequest.ownerName}</span>
                                        </div>
                                        <div className="flex flex-col">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-gray-700 break-all">{selectedRequest.email}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div className="p-4 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500">Visiting Card / Shop Photo</p>
                                            </div>
                                        </div>
                                        <a
...
export default PrintPartnerRequestsAdmin;
