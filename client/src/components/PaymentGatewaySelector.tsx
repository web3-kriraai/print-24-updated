...
const PaymentGatewaySelector: React.FC<PaymentGatewaySelectorProps> = ({
    ...
    const fetchGateways = async () => {
        try {
            ...
            const response = await fetch(`${(import.meta as any).env.VITE_API_BASE_URL}/api/admin/payment-gateways`, {
                headers: {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            // Initialize payment
            const response = await fetch(`${(import.meta as any).env.VITE_API_BASE_URL}/api/payment/initialize`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
    ...
    const showRazorpayCheckout = async (paymentData: any) => {
        ...
        const options = {
            ...
            handler: async function (response: any) {
                ...
                try {
                    const token = localStorage.getItem('token');
                    const verifyResponse = await fetch(`${(import.meta as any).env.VITE_API_BASE_URL}/api/payment/verify`, {
                        method: 'POST',
                        headers: {
                            'Authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json'
